...
import { useTimeStore, getHours, getMinutes, getAmpm, getDateString, getIsNight } from '../stores/time'
...
export default function MacSimulator() {
  ...
  const calculateHeroClipPath = useCallback(() => {
    ...
    const r = 40
    setHeroClipPath(`M ${r} 0 L ${w - r} 0 Q ${w} 0 ${w} ${r} L ${w} ${h - r} Q ${w} ${h} ${w - r} ${h} L ${r} ${h} Q 0 ${h} 0 ${h - r} L 0 ${r} Q 0 0 ${r} 0 Z`)
  }, [])
  ...
  const handleUnlockClick = useCallback(() => {
    ...
    const t1 = setTimeout(() => {
      emitter.emit('toggle-notification', { state: true, type: 'didConnectAirPods', duration: 3500 })
  ...
  }, [handleScroll, calculateHeroClipPath, startTime, stopTime, getTracks, dispose])
      ...
      >
        ...
        <div className="absolute top-0 right-0 left-0 z-20 flex justify-center sm:grid sm:grid-cols-3">
          ...
          <div className="flex p-5">
            <div
              className={`flex h-10 items-center rounded-[0.625rem] rounded-tl-[1.25rem] bg-orange-50/10 backdrop-blur-xl backdrop-saturate-115 transition-all duration-500 ${isLocked ? 'opacity-0 -translate-y-1' : 'opacity-100 translate-y-0'}`}
            >
              <div className="flex items-center space-x-2 px-3 text-orange-50">
                <svg className="size-[15px] fill-current opacity-80" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" aria-hidden="true">
                ...
                <span className="text-[15px] font-medium whitespace-nowrap">Work</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center p-5">
            <DynamicIsland isLocked={isLocked} notchWidth={136} notchHeight={40} />
          </div>

          {/* Right pill: time + battery */}
          <div className="flex justify-end p-5">
            <div
              className={`flex h-10 items-center justify-center space-x-5 rounded-[0.625rem] rounded-tr-[1.25rem] bg-orange-50/10 px-5 text-orange-50 backdrop-blur-xl backdrop-saturate-115 transition-all duration-500 ${isLocked ? 'opacity-0 -translate-y-1' : 'opacity-100 translate-y-0'}`}
            >
              <span className="text-[15px] font-medium whitespace-nowrap">
                {hours}:{minutes}
                <span className="hidden md:inline"> {ampm}</span>
              </span>
              <svg className="size-[22px] fill-current opacity-80" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" aria-hidden="true">
                <path d="M3.5 9.5A2.5 2.5 0 0 1 6 7h14a2.5 2.5 0 0 1 2.5 2.5v9A2.5 2.5 0 0 1 20 21H6a2.5 2.5 0 0 1-2.5-2.5v-9Zm1.5 0v9A1 1 0 0 0 6 19.5h14a1 1 0 0 0 1-1v-9A1 1 0 0 0 20 8.5H6A1 1 0 0 0 5 9.5Zm17 3h1a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1h-1v-3Zm-15.5 1a1 1 0 0 1 1-1h8a1 1 0 0 1 0 2h-8a1 1 0 0 1-1-1Z" />
              </svg>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0">
          {/* Bottom app dock (2xl only) */}
          <div className="absolute bottom-24 z-20 hidden w-full items-center justify-center 2xl:flex">
            <div
              className={`relative flex items-center justify-center rounded-3xl bg-orange-50/15 pt-[11px] pr-3 pb-3 pl-2.5 before:absolute before:inset-0 before:-z-10 before:rounded-3xl before:border-t before:border-orange-50/15 before:backdrop-blur-xl before:backdrop-saturate-115 transition-all duration-500 delay-300 ${isLocked ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}
            >
              <div className="size-16">
                <picture>
              <source srcSet="/images/icon.webp" type="image/webp" />
              <img className="size-full object-contain select-none" src="/images/icon.png" alt="Dyno" draggable={false} />
                </picture>
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex flex-col items-center">
              <div className="hidden flex-col items-center text-center lg:flex">
                ...
                <div className="font-numeric text-8xl font-bold text-purple-300/90 saturate-150">{hours}:{minutes}</div>
              </div>
              ...
              <div className="mt-3.5 hidden items-center space-x-5 font-semibold text-purple-300/90 saturate-150 lg:flex">
                ...
                <div className="flex items-center">
                  ...
                  <div className="relative inline-flex tabular-nums">
                    <span>{currentTemperature}</span><span>&deg;</span>
                  </div>
                </div>
              </div>
            </div>
